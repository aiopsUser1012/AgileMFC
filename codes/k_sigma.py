...
#             config['MIN_TEST_LENGTH'] = 5
    ...
    def detectionv1(self, data: list, start_ts: int, end_ts: int):
        """
        return
            is_anomalous[True or False], anomaly_start_ts[ts], anomaly_score
        """
        ...
        if str(start_ts) not in ts_list or str(end_ts) not in ts_list:
            return False, None, 0
        
        st_index = ts_list.index(str(start_ts))
        end_index = ts_list.index(str(end_ts))

        test_data = data[st_index: end_index]
        win_size, k_thr, std_thr = config['k_s']['win_size'], config['k_s']['k_thr'], config['k_s']['std_thr']
        del config
        if len(test_data) < win_size:
            return False, None, 0

        values = [float(i[1]) for i in test_data]
        timestamps = [i[0] for i in test_data]
